            ...
            <table class="w-full border border-collapse border-gray-400 table-auto ">
              ...
              <tbody class="bg-white">
                ...
                <tr class="{{ $loop->even ? 'bg-gray-100 hover:bg-gray-300' : 'hover:bg-gray-300'}}">
                  <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-300"><form action="/list/{{ $student->id }}" method="post" > @csrf {{ method_field('PUT') }}{{ $student->name }}</td>
                  ...
                  <td class="px-6 py-4 text-base text-gray-600 whitespace-no-wrap border-b border-gray-300">{{ $student->helper != null ?
                    $helpers->find($student->helper) != null ? $helpers->find($student->helper)->name : 'Ištrintas' : 'Nėra' }}</td>
                    {{-- Naujas pagalbininkas --}}
                  <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-300">
                    <x-helper-select :helpers="$helpers" :student="$student" />
                  </td>
                  <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-300"><button type="submit" class="p-2 text-white bg-indigo-500 rounded-md shadow-sm hover:bg-indigo-600">Patvirtinti</button></form></td>
                </tr>
                @endforeach
                </form>
              </tbody>
